...
execute as @e[tag=camera, type=armor_stand] if score @s camera = #teleport_camera camera run tag @s add camera_destination
execute unless entity @e[tag=camera_destination, type=armor_stand, limit=1] run return run tellraw @s ["◎ 監視器", {score: {name: "#teleport_camera", objective: "camera"}}, {text: "已損壞", color: "gold"}]
...
#傳送到目標監視器
execute rotated as @e[tag=camera_destination, type=armor_stand, limit=1] run rotate @s ~ ~
ride @s dismount
ride @s mount @e[tag=camera_destination, type=armor_stand, limit=1]
tag @e[tag=camera_destination, type=armor_stand, limit=1] remove camera_destination