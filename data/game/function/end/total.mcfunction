...
tp @a @e[tag=lobby_spawn, type=marker, limit=1]
gamemode adventure @a
scoreboard players set #mode system 2
execute at @e[tag=lobby_spawn, type=marker, limit=1] run spawnpoint @s ~ ~ ~

effect clear @a
effect give @a resistance infinite 4 true
effect give @a saturation infinite 0 true
effect give @a night_vision infinite 0 true
effect give @a weakness infinite 99 true

give @a written_book[written_book_content={pages:\
[[\
	"",{"text":"【總分數】",click_event:{"action":"run_command","command":"/scoreboard objectives setdisplay sidebar end"}},{"text":"\n"},\
