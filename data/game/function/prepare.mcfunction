...
#清空大廳
execute at @e[tag=map_info, type=marker, limit=1] run setblock ~ ~ ~ air
execute at @e[tag=monster_info, type=marker, limit=1] run setblock ~ ~ ~ air
kill @e[tag=lobby_spawn,type=marker]
kill @e[tag=lobby_text]
kill @e[tag=lobby_setting]
kill @e[tag=monitor_marker]
execute at @e[tag=lobby, type=marker, limit=1] run place template game:monitor ~-10 ~-1 ~-10

#發電
scoreboard players set @e[tag=power, type=marker] power_level 1
...
#設定初始怪物
execute as @e[tag=summon_pick,type=armor_stand,scores={summon_number=1..7}] run function summon:pick/setting

#重置收容
kill @e[tag=contain]
kill @e[tag=contain_escape]
execute as @e[tag=contain_marker,type=marker] at @s run function contain:summon
...
execute at @e[tag=spawn_survivor,type=marker,limit=1] run setblock ~ ~ ~ ender_chest
execute at @e[tag=spawn_survivor,type=marker,limit=1] run spawnpoint @a[team=survivor]
execute at @e[tag=lobby,type=marker,limit=1] run spawnpoint @a[team=monitor]
