scoreboard players set @a puzzle_game 0
$execute unless data block ~ ~ ~ Items[{Slot:$(check)b}] store success score @s puzzle_game run clear @s echo_shard[custom_data~{number:$(check)}] 0
$execute as @a[scores={puzzle_game=1}] run data modify storage jk2:data root.monitor.puzzle_game.switch set value $(check)
