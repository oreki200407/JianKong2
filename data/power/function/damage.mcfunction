...
setblock ~ ~1 ~ oak_wall_sign[facing=south]{front_text:{messages:["",{"text":"修復發電機","bold":true,"click_event":{"action":"run_command","command":"execute if score 電力 info_monitor matches ..29 run return run tellraw @s [\"◎ 電力不足, \", {\"text\": \"無法修復\", \"color\":\"gold\"}]"}},{"text":"花費: 30電力","bold":true,"click_event":{"action":"run_command","command":"execute if score 電力 info_monitor matches 30.. run function power:fix_buy"}},""]},is_waxed:1b}
